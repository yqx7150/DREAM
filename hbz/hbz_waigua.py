...
class GetMRI(data.Dataset):
    def __init__(self,root,root2 = None,root3 = None,augment=None):
        super().__init__()
        self.data_names = np.array([root+"/"+x  for x in sorted(os.listdir(root))])
        self.augment = None
        if root2 is not None:
            self.data_names2 = np.array([root2+"/"+x  for x in sorted(os.listdir(root2))])
        if root3 is not None:
            self.data_names3 = np.array([root3+"/"+x  for x in sorted(os.listdir(root3))])
    ...
    def __getitem__(self,index):
        ...
        ori_data_img=cv2.imread(img_path)
        # try:
        #     ori_data_img = loadmat(img_path)['Img']
        # except:
        #     ori_data_img = loadmat(img_path)['DATA']
        '''
        ...
        return {'Img_path':img_path,  'Img_data':ori_data_img}
    
    def __len__(self):
        return len(self.data_names)


def get_dataset(root,root2 = None, root3 = None):
