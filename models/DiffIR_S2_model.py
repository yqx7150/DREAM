...
class DiffIRS2Model(SRModel):
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img,opt = None):
        # do not use the synthetic process during validation
        self.is_train = False
        super(DiffIRS2Model, self).nondist_validation(dataloader, current_iter, tb_logger, save_img,opt = opt)
        self.is_train = True
    ...
    def test(self,mode=None): # S2训练的val        最终test
        # print("S2_model")
        ...
        if window_size:
            ...
            lq=self.lq
            gt=self.gt
        if hasattr(self, 'net_g_ema'):# S2训练的val
            # print("S2ema@@@@@@@@@")
            self.net_g_ema.eval()
            with torch.no_grad():
            ...
            with torch.no_grad():
                # if mode == "test":
                #     data = self.lq * 2
                #     self.output = torch.zeros_like(data).to(self.device)
                #     for i in range(5184//3):
                #         self.output[:,3*i:3*i+3,...] = self.net_g(data[:,3*i:3*i+3,...])
    ...
    def test_WT(self,mode=None):# 对不对未知  待定
        import pywt
        print("S2_model_wt")
        window_size = self.opt['val'].get('window_size', 0)
        if window_size:
        ...
        if hasattr(self, 'net_g_ema'):
            ...
            with torch.no_grad():
                if mode == "test":
                    ...
                    self.output = []
                    for i in range(3):
                        # coeffs = pywt.dwt2(self.gt.squeeze()[i,:,:], 'haar')
                        # cA, (cH, cV, cD) = coeffs
                        coeffs2 = pywt.dwt2(self.lq.squeeze()[i,:,:], 'haar')
                        cA2, (cH2, cV2, cD2) = coeffs2
                        # GT_wt = torch.stack((cA, cH, cV, cD),0)
                        LQ_wt = torch.stack((cA2, cH2, cV2, cD2),0)

                        output = self.net_g(LQ_wt[None,...])

                        a = output.squeeze()[0,:,:],(output.squeeze()[1,:,:],output.squeeze()[2,:,:],output.squeeze()[3,:,:])
                        self.output.append(pywt.idwt2(a, 'haar'))

                    self.output = torch.stack(self.output,0)[None,...]
                    # self.output = self.net_g(self.lq)
                # self.output = self.net_g(lq)
            self.net_g.train()
        if window_size:
